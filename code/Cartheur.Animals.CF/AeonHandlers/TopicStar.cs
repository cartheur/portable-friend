        ...
        protected override string ProcessChange()
        {
            if (TemplateNode.Name.ToLower() == "topicstar")
            {
                if (TemplateNode.Attributes != null && TemplateNode.Attributes.Count == 0)
                {
                    ...
                    ThisAeon.WriteToLog("An out of bounds index to topicstar was encountered when processing the input: " + UserRequest.RawInput, Logging.LogType.Error, Logging.LogCaller.TopicStar);
                }
                        ...
                        if (TemplateNode.Attributes[0].Value.Length > 0)
                        {
                            try
                            {
                                ...
                                if (Query.TopicStar.Count > 0)
                                {
                                    ...
                                    ThisAeon.WriteToLog("An input tag with a badly formed index (" + TemplateNode.Attributes[0].Value + ") was encountered processing the input: " + UserRequest.RawInput, Logging.LogType.Error, Logging.LogCaller.TopicStar);
                                }
                                else
                                {
                                    ThisAeon.WriteToLog("An out of bounds index to topicstar was encountered when processing the input: " + UserRequest.RawInput, Logging.LogType.Error, Logging.LogCaller.TopicStar);
                            ...
                            {
                                ThisAeon.WriteToLog("A thatstar tag with a badly formed index (" + TemplateNode.Attributes[0].Value + ") was encountered processing the input: " + UserRequest.RawInput, Logging.LogType.Error, Logging.LogCaller.TopicStar);
                            }
                        }
                    }
                }
